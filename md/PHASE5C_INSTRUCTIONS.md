...
---
## 1. Executive Summary
Phase 5C elevates configuration from static structs + runtime adjustments to a *governed configuration platform*. It enables:
...
|----------|-----------|------------------|
| Hierarchy | Layered resolution implemented (at least 5 layers) | Unit tests (resolution matrix) |
...
| Hot Reload | Atomic apply with rollback on validation failure | Integration test with induced failure |
...
| API | Internal Go API + CLI entrypoint for apply/simulate | CLI test exercises commands |
...
| Docs | Operator & developer guides completed | Markdown existence + checklist |
...
- Basic canary: percentage of *domains* or *site groups*
- Emission of structured change events (logger + Prometheus counter + trace annotation)
- CLI: `config apply`, `config simulate`, `config diff`, `config rollback`, `config show`
...
```
packages/engine/configx/              # New advanced configuration subsystem
  ...
  api.go                              # Public API surface (used by engine)
  cli.go                              # Wiring for CLI commands
...
---
## 6. CLI Commands (Minimum Set)
| Command | Purpose | Notes |
|---------|---------|-------|
| `config show [--version N]` | Display active or specific version | JSON or table output |
| `config apply -f file.yaml` | Apply new config | Supports `--actor`, `--dry-run`, `--force` |
| `config simulate -f file.yaml` | Run simulation only | Returns JSON impact report |
| `config diff --from A --to B` | Show diff between versions | Field-level summary |
| `config rollback --to N` | Roll back to prior version | Creates new version with rollback reason |
| `config rollout promote` | Promote staged cohort rollout to full | Updates rollout spec |
...
| 7 | CLI wiring | cli.go + command registration |
| 8 | Hardening & edge cases | corruption tests, race detector run |
...
| `config-operations-guide.md` | Operator runbook (apply, simulate, rollback, recovery) |
...
- [ ] 90%+ test coverage for `configx` package (excluding CLI glue)
- [ ] Zero lint issues (golangci-lint)
- [ ] Race detector clean (`go test -race ./...` for new code)
- [ ] Documentation deliverables present & peer-reviewed
