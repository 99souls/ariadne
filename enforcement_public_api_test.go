...
func TestNoInternalImportsOutsideEngine(t *testing.T) {
    ...
    err := filepath.WalkDir(".", func(path string, d os.DirEntry, err error) error {
        ...
        if d.IsDir() {
            if strings.Contains(path, "engine/internal") { return filepath.SkipDir }
            // skip VCS & vendor
            base := filepath.Base(path)
            if strings.HasPrefix(base, ".git") || base == "vendor" { return filepath.SkipDir }
            return nil
        }
...
func TestNoNewTopLevelDirs(t *testing.T) {
    allowed := map[string]struct{}{"engine": {}, "cli": {}, "md": {}, ".github": {}, ".git": {}}
    transitional := map[string]struct{}{"internal": {}, "cmd": {}, "test": {}}
    entries, err := os.ReadDir("."); if err != nil { t.Fatalf("read root: %v", err) }
    for _, e := range entries { if !e.IsDir() { continue }; name := e.Name(); if _, ok := allowed[name]; ok { continue }; if _, ok := transitional[name]; ok { continue }; t.Fatalf("unexpected new top-level directory: %s", name) }
}
