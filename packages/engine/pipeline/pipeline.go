...
func (p *Pipeline) processContent(page *engmodels.Page) *engmodels.CrawlResult { time.Sleep(5 * time.Millisecond); var processedPage *engmodels.Page; if page != nil { crawledAt := time.Unix(page.CrawledAt.Unix(), page.CrawledAt.UnixNano()%1e9); processedPage = &engmodels.Page{ URL: page.URL, Title: page.Title, Content: page.Content, CleanedText: page.CleanedText, Markdown: page.Markdown, Links: page.Links, Images: page.Images, Metadata: page.Metadata, CrawledAt: crawledAt, ProcessedAt: time.Now() } }; resultURL := ""; if processedPage != nil && processedPage.URL != nil { resultURL = processedPage.URL.String() }; return &engmodels.CrawlResult{ URL: resultURL, Page: processedPage, Success: true, Stage: "processing" } }
