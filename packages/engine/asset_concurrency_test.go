...
func TestAssetExecuteConcurrency(t *testing.T) {
	...
	fetchAsset = func(ctx context.Context, rawURL string, capRemaining int64) ([]byte, error) {
		...
		if regexp.MustCompile(`slow`).MatchString(rawURL) { time.Sleep(10 * time.Millisecond) }
	...
	if mut.Content == html { t.Fatalf("expected rewrite mutation") }
	...
	for _, m := range matches { set[m] = struct{}{} }
	if len(set) != 5 {
		keys := make([]string,0,len(set)); for k := range set { keys = append(keys,k) }; sort.Strings(keys)
	...
	if snap.Discovered < 5 || snap.Selected < 5 || snap.Downloaded < 5 {
		...
		if snap.Downloaded < 5 { t.Errorf("expected >=5 downloaded, got %d", snap.Downloaded) }
	}
}
...
func BenchmarkAssetExecute(b *testing.B) {
	origFetch := fetchAsset
	fetchAsset = func(ctx context.Context, rawURL string, capRemaining int64) ([]byte, error) { return []byte("data"), nil }
	defer func() { fetchAsset = origFetch }()
	...
	for i := 0; i < 32; i++ { base += `<img src="/img/one` + strconv.Itoa(i%10) + `.png">` }
	base += `</body></html>`
