...
func (a *Applier) Apply(current *EngineConfigSpec, candidate *EngineConfigSpec, opts ApplyOptions) (*ApplyResult, error) {
	if err := ValidateSpec(candidate); err != nil {
		a.observeFailure("validation_error", 0, opts.Actor, err)
		return nil, err
	}
	...
	if a.Simulator != nil {
		...
		if !impact.Acceptable && !opts.Force && !opts.DryRun {
			a.observeFailure("simulation_reject", 0, opts.Actor, ErrSimulationRejected)
	...
	if err != nil {
		a.observeFailure("append_error", 0, opts.Actor, err)
		return nil, err
	}
...
func (a *Applier) observeFailure(kind string, version int64, actor string, err error) {
	...
	if a.Dispatcher != nil {
		a.Dispatcher.Emit(ChangeEvent{Type: kind, Version: version, Actor: actor, Error: err, Timestamp: time.Now().UTC()})
	}
}
