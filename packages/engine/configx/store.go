...
func (s *VersionedStore) Get(version int64) (*VersionedConfig, bool) {
	if version <= 0 { return nil, false }
	s.mu.RLock(); defer s.mu.RUnlock()
	if int(version) > len(s.versions) { return nil, false }
...
func (s *VersionedStore) Head() (*VersionedConfig, bool) {
	s.mu.RLock(); defer s.mu.RUnlock()
	if len(s.versions) == 0 { return nil, false }
...
func (s *VersionedStore) Append(spec *EngineConfigSpec, actor, diff string, parentExpected int64) (*VersionedConfig, error) {
	if spec == nil { return nil, errors.New("nil spec") }
	...
	var parent int64
	if len(s.versions) > 0 { parent = s.versions[len(s.versions)-1].Version }
...
func (s *VersionedStore) Verify(version int64) error {
	...
	h := sha256.Sum256(raw)
	hash := hex.EncodeToString(h[:])
	if hash != vc.Hash { return ErrHashMismatch }
	return nil
}
...
func cloneSpec(spec *EngineConfigSpec) *EngineConfigSpec {
	...
	c := *spec
	if spec.Global != nil { g := *spec.Global; if spec.Global.RetryPolicy != nil { rp := *spec.Global.RetryPolicy; g.RetryPolicy = &rp }; c.Global = &g }
	if spec.Crawling != nil {
		cr := *spec.Crawling
		if cr.SiteRules != nil { cr.SiteRules = cloneSiteRulesMap(cr.SiteRules) }
		if cr.LinkRules != nil { lr := *cr.LinkRules; cr.LinkRules = &lr }
		if cr.RateRules != nil { rr := *cr.RateRules; if rr.SiteDelays != nil { sd := make(map[string]time.Duration, len(rr.SiteDelays)); for k,v := range rr.SiteDelays { sd[k]=v }; rr.SiteDelays = sd }; cr.RateRules = &rr }
		c.Crawling = &cr
	}
	if spec.Processing != nil { pr := *spec.Processing; if len(pr.ExtractionRules)>0 { pr.ExtractionRules = cloneStringSlice(pr.ExtractionRules)}; if len(pr.ProcessingSteps)>0 { pr.ProcessingSteps = cloneStringSlice(pr.ProcessingSteps)}; if pr.ConditionalActions!=nil { m:=make(map[string]string,len(pr.ConditionalActions)); for k,v:= range pr.ConditionalActions {m[k]=v}; pr.ConditionalActions=m}; c.Processing=&pr }
	if spec.Output != nil { o := *spec.Output; if o.RoutingRules!=nil { m:=make(map[string]string,len(o.RoutingRules)); for k,v:=range o.RoutingRules {m[k]=v}; o.RoutingRules=m }; if len(o.QualityGates)>0 { o.QualityGates = cloneStringSlice(o.QualityGates)}; c.Output=&o }
	if spec.Policies != nil { p := *spec.Policies; if p.BusinessRules!=nil { br := make([]*PolicyRuleSpec,0,len(p.BusinessRules)); for _,r:=range p.BusinessRules { if r==nil {continue}; rr:=*r; br=append(br,&rr)}; p.BusinessRules=br }; if p.EnabledFlags!=nil { ef:=make(map[string]bool,len(p.EnabledFlags)); for k,v:=range p.EnabledFlags {ef[k]=v}; p.EnabledFlags=ef }; c.Policies=&p }
	if spec.Rollout != nil { r := *spec.Rollout; if len(r.CohortDomains)>0 { r.CohortDomains = cloneStringSlice(r.CohortDomains)}; if len(r.CohortDomainGlobs)>0 { r.CohortDomainGlobs = cloneStringSlice(r.CohortDomainGlobs)}; c.Rollout=&r }
	return &c
}

func cloneSiteRulesMap(m map[string]*SiteCrawlerRule) map[string]*SiteCrawlerRule {
	out := make(map[string]*SiteCrawlerRule, len(m))
	for k, v := range m { out[k] = cloneSiteRule(v) }
	return out
}
