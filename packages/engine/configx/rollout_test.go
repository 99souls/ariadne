...
func TestRolloutEvaluator_Full(t *testing.T) {
    ...
    vc, err := s.Append(spec, "actor", "", 0); if err != nil { t.Fatalf("append: %v", err) }
    ev := NewRolloutEvaluator(s)
    if got := ev.ActiveVersionForDomain("any.com"); got != vc.Version { t.Fatalf("expected head version") }
}
...
func TestRolloutEvaluator_Percentage(t *testing.T) {
    ...
    for _, d := range domains {
        ...
        if v == head.Version { sawHead = true } else if v == base.Version { sawBase = true } else { t.Fatalf("unexpected version %d for domain %s", v, d) }
    }
    if !sawBase || !sawHead { t.Fatalf("expected mixture base=%v head=%v", sawBase, sawHead) }
}
...
func TestRolloutEvaluator_Cohort(t *testing.T) {
    ...
    ev := NewRolloutEvaluator(s)
    if v := ev.ActiveVersionForDomain("target.com"); v != head.Version { t.Fatalf("target domain should get head version") }
    if v := ev.ActiveVersionForDomain("other.com"); v != base.Version { t.Fatalf("non-cohort domain should get base version got %d want %d", v, base.Version) }
}