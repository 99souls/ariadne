...
func TestStoreHashCorruption(t *testing.T) {
    ...
    if err != nil { t.Fatalf("append err: %v", err) }
    if err := s.Verify(vc.Version); err != nil { t.Fatalf("verify failed unexpectedly: %v", err) }
    // Tamper internal hash (tests share package so can access)
    s.mu.Lock()
    s.versions[vc.Version-1].Hash = "deadbeef"
    s.mu.Unlock()
    if err := s.Verify(vc.Version); err == nil || err != ErrHashMismatch { t.Fatalf("expected hash mismatch, got %v", err) }
}
...
func TestApplyNilSpec(t *testing.T) {
    ...
    if err == nil { t.Fatalf("expected error for nil spec") }
}
...
func TestRollbackNonexistent(t *testing.T) {
    s := NewVersionedStore()
    a := NewApplier(s, NewSimulator())
    if _, err := a.Rollback(99, "actor"); err == nil { t.Fatalf("expected error rollback nonexistent version") }
}
...
func TestConcurrentAccess(t *testing.T) {
    ...
    if _, err := a.Apply(nil, base, ApplyOptions{Actor: "init"}); err != nil { t.Fatalf("init apply err: %v", err) }
    ...
    for i := 0; i < 10; i++ {
        ...
        go func(i int) {
            ...
            a.Apply(nil, spec, ApplyOptions{Actor: "c"}) // ignore errors (simulation always acceptable)
        }(i)
        wg.Add(1)
        go func() {
            defer wg.Done()
