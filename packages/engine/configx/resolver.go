...
func mergeSpecs(dst, src *EngineConfigSpec) {
	...
	if src.Crawling != nil {
		if dst.Crawling == nil {
			dst.Crawling = &CrawlingConfigSection{}
		}
		mergeCrawling(dst.Crawling, src.Crawling)
	}
	if src.Processing != nil {
		if dst.Processing == nil {
			dst.Processing = &ProcessingConfigSection{}
		}
		mergeProcessing(dst.Processing, src.Processing)
	}
	if src.Output != nil {
		if dst.Output == nil {
			dst.Output = &OutputConfigSection{}
		}
...
func mergeCrawling(dst, src *CrawlingConfigSection) {
	if src.SiteRules != nil {
		if dst.SiteRules == nil {
			dst.SiteRules = make(map[string]*SiteCrawlerRule, len(src.SiteRules))
		}
	...
	if src.LinkRules != nil {
		if dst.LinkRules == nil {
			dst.LinkRules = &LinkRuleConfig{}
		}
	...
	if src.RateRules != nil {
		if dst.RateRules == nil {
			dst.RateRules = &RateLimitConfig{}
		}
		if src.RateRules.DefaultDelay != 0 || dst.RateRules.DefaultDelay == 0 {
			dst.RateRules.DefaultDelay = src.RateRules.DefaultDelay
		}
		if src.RateRules.SiteDelays != nil {
			if dst.RateRules.SiteDelays == nil {
				dst.RateRules.SiteDelays = make(map[string]time.Duration, len(src.RateRules.SiteDelays))
			}
...
func mergeProcessing(dst, src *ProcessingConfigSection) {
	if len(src.ExtractionRules) > 0 {
		dst.ExtractionRules = cloneStringSlice(src.ExtractionRules)
	}
	if src.QualityThreshold != 0 || dst.QualityThreshold == 0 {
		dst.QualityThreshold = src.QualityThreshold
	}
	if len(src.ProcessingSteps) > 0 {
		dst.ProcessingSteps = cloneStringSlice(src.ProcessingSteps)
	}
...
func cloneSiteRule(r *SiteCrawlerRule) *SiteCrawlerRule {
	...
	if len(r.AllowedDomains) > 0 {
		c.AllowedDomains = cloneStringSlice(r.AllowedDomains)
	}
	if len(r.Selectors) > 0 {
		c.Selectors = cloneStringSlice(r.Selectors)
	}
	return &c
}
