...
func (p *PrometheusProvider) buildFQName(c CommonOpts) (string, error) {
    if c.Name == "" { return "", errors.New("metric name required") }
    // Prometheus fqname rules: optional namespace_subsystem_name
    parts := []string{}
    if c.Namespace != "" { parts = append(parts, c.Namespace) }
    if c.Subsystem != "" { parts = append(parts, c.Subsystem) }
    parts = append(parts, c.Name)
    fq := parts[0]
    for i:=1;i<len(parts);i++ { fq += "_"+parts[i] }
...
func (p *PrometheusProvider) NewCounter(opts CounterOpts) Counter {
    ...
    p.mu.RLock(); cv := p.counters[fq]; p.mu.RUnlock()
    if cv != nil { return &promCounter{cv: cv, provider: p, id: fq} }
    ...
    p.mu.Lock(); p.counters[fq]=vec; p.mu.Unlock()
...
func (p *PrometheusProvider) NewGauge(opts GaugeOpts) Gauge {
    ...
    p.mu.RLock(); gv := p.gauges[fq]; p.mu.RUnlock()
    if gv != nil { return &promGauge{gv: gv, provider: p, id: fq} }
    ...
    p.mu.Lock(); p.gauges[fq]=vec; p.mu.Unlock()
...
func (p *PrometheusProvider) NewHistogram(opts HistogramOpts) Histogram {
    ...
    p.mu.RLock(); hv := p.histograms[fq]; p.mu.RUnlock()
    if hv != nil { return &promHistogram{hv: hv, provider: p, id: fq} }
    ...
    p.mu.Lock(); p.histograms[fq]=vec; p.mu.Unlock()
...
func (p *PrometheusProvider) Health(ctx context.Context) error {
    ...
    if len(p.problems) == 0 { return nil }
...
type promCounter struct { cv *prom.CounterVec; provider *PrometheusProvider; id string }
...
type promGauge struct { gv *prom.GaugeVec; provider *PrometheusProvider; id string }
...
type promHistogram struct { hv *prom.HistogramVec; provider *PrometheusProvider; id string }
...
type promTimer struct { hist Histogram; start time.Time }
