...
func TestAssetInstrumentationAndDeterminism(t *testing.T) {
    ...
    if !reCSS.MatchString(mutated1.Content) { t.Errorf("missing rewritten css path") }
    if !reJS.MatchString(mutated1.Content) { t.Errorf("missing rewritten js path") }
    if !reSVG.MatchString(mutated1.Content) { t.Errorf("missing rewritten svg path") }
    ...
    for _, ev := range events {
        if ev.Type == "asset_download" { downloads++ }
        if ev.Type == "asset_optimize" { optimizes++ }
    }
    if downloads != 3 { t.Errorf("expected 3 download events, got %d", downloads) }
    if optimizes != 3 { t.Errorf("expected 3 optimize events, got %d", optimizes) }
    ...
    if err != nil { t.Fatalf("second hook run error: %v", err) }
    if mutated2.Content != mutated1.Content { t.Errorf("expected deterministic rewrite; contents differ") }
}
