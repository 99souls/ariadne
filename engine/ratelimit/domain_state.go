...
func (ds *domainState) allowRequestLocked(cfg engmodels.RateLimitConfig, now time.Time) bool {
	ds.lastActivity = now
...
func (ds *domainState) planRequest(cfg engmodels.RateLimitConfig, now time.Time) (time.Duration, error) {
	ds.mu.Lock()
	defer ds.mu.Unlock()
...
func (ds *domainState) updateBreakerAfterFeedback(cfg engmodels.RateLimitConfig, now time.Time, isError bool, success bool, errorRate float64, total int) {
	switch ds.breaker.state {
	case circuitClosed:
		...
		if (cfg.ErrorRateThreshold > 0 && total >= minSamples && errorRate >= cfg.ErrorRateThreshold) || (cfg.ConsecutiveFailThreshold > 0 && ds.breaker.consecutiveFails >= cfg.ConsecutiveFailThreshold) {
			ds.openBreaker(now)
		}
	case circuitOpen:
		if now.Sub(ds.breaker.openedAt) >= effectiveOpenDuration(cfg.OpenStateDuration) {
