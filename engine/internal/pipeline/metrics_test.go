...
func TestPipelineMetricsSnapshot(t *testing.T) {
	...
	m := pl.Metrics()
	if m.TotalProcessed < len(urls) {
		t.Fatalf("expected >=%d processed got %d", len(urls), m.TotalProcessed)
	}
	if m.Duration <= 0 {
		t.Fatalf("expected duration > 0")
	}
	for _, stage := range []string{"discovery", "extraction", "processing", "output"} {
		if m.StageMetrics[stage].Processed == 0 {
			t.Fatalf("stage %s zero processed", stage)
		}
	}
}
