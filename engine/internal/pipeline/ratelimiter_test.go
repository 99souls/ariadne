...
import (
	...
	engratelimit "github.com/99souls/ariadne/engine/internal/ratelimit"
)
...
func (s *stubLimiter) Acquire(ctx context.Context, domain string) (engratelimit.Permit, error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	cnt := s.attempts[domain] + 1
	s.attempts[domain] = cnt
	if cnt == 1 {
		return nil, engratelimit.ErrCircuitOpen
	}
...
func TestPipelineRateLimiterIntegration(t *testing.T) {
	...
	if lim.Attempts("example.com") < 2 {
		t.Fatalf("expected limiter to retry after open circuit")
	}
}
