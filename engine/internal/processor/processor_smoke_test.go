...
func TestProcessPageSmoke(t *testing.T) {
    ...
    if err := cp.ProcessPage(p, "https://example.com"); err != nil { t.Fatalf("process: %v", err) }
    if p.Markdown == "" { t.Fatalf("expected markdown output") }
    if p.Metadata.WordCount == 0 { t.Fatalf("expected word count") }
}
