...
func TestLiveSiteLargeAssetThroughput(t *testing.T) {
    ...
    testsite.WithLiveTestSite(t, func(base string) {
        ...
        if err != nil { t.Fatalf("parse base: %v", err) }
        ...
        for time.Now().Before(deadline) {
            ...
            case r := <-crawler.Results():
                if r == nil { break }
                if r.URL == largeURL && r.Stage == "asset" && r.StatusCode == 200 { sawLarge = true }
                if r.URL == base+"/about" && r.Success { sawAbout = true }
            case <-time.After(100 * time.Millisecond):
            }
            if sawLarge && sawAbout { break }
        }
        crawler.Stop()
        dur := time.Since(start)
        if !sawLarge { t.Fatalf("did not fetch large asset %s", largeURL) }
        if !sawAbout { t.Fatalf("did not discover normal page /about while large asset in flight") }
        if dur > 2500*time.Millisecond { t.Fatalf("crawl took too long with large asset present: %v", dur) }
    })
}
