...
import (
	...
	engratelimit "ariadne/engine/ratelimit"
...
type Pipeline struct {
	...
	processingQueue  chan *engmodels.Page
	outputQueue      chan *engmodels.CrawlResult
	resultsInternal  chan *engmodels.CrawlResult
	results          chan *engmodels.CrawlResult
	ctx              context.Context
...
func (p *Pipeline) ProcessURLs(ctx context.Context, urls []string) <-chan *engmodels.CrawlResult {
...
func (p *Pipeline) initStageStatus() {
	stages := map[string]int{"discovery": p.config.DiscoveryWorkers, "extraction": p.config.ExtractionWorkers, "processing": p.config.ProcessingWorkers, "output": p.config.OutputWorkers}
	for name, workers := range stages {
		p.stageStatus[name] = &StageStatus{Name: name, Workers: workers, Active: true, Queue: 0}
	}
	if p.resourceManager != nil {
		p.stageStatus["cache"] = &StageStatus{Name: "cache", Workers: 0, Active: true, Queue: 0}
	}
}
...
func (p *Pipeline) forwardResult(result *engmodels.CrawlResult) bool {
	select {
	case <-p.ctx.Done():
		return false
...
func (p *Pipeline) deliverResult(result *engmodels.CrawlResult) bool {
	select {
	case <-p.ctx.Done():
		return false
...
func (p *Pipeline) forwardToProcessing(page *engmodels.Page, fromCache bool) bool {
	if page == nil {
		return false
	}
	select {
...
func (p *Pipeline) acquirePermit(task extractionTask, domain string) (engratelimit.Permit, error) {
	...
	for {
		...
		case task, ok := <-p.extractionQueue:
			...
			if err != nil {
				...
				p.updateStageMetrics("extraction", false)
				if errors.Is(err, engratelimit.ErrCircuitOpen) && p.shouldRetry(task) {
			...
			if page != nil {
				...
				releaseSlot()
				if p.limiter != nil && domain != "" {
					p.limiter.Feedback(domain, engratelimit.Feedback{StatusCode: 200, Latency: latency})
				}
				...
				releaseSlot()
				if p.limiter != nil && domain != "" {
					p.limiter.Feedback(domain, engratelimit.Feedback{StatusCode: 503, Latency: latency, Err: errors.New("extraction failed")})
				}
				p.updateStageMetrics("extraction", false)
...
func (p *Pipeline) extractContent(rawURL string) *engmodels.Page {
	...
	page := &engmodels.Page{Title: "Test Page", Content: "<h1>Test Content</h1>"}
...
func (p *Pipeline) processContent(page *engmodels.Page) *engmodels.CrawlResult {
	time.Sleep(5 * time.Millisecond)
	var processedPage *engmodels.Page
	if page != nil {
	...
	return &engmodels.CrawlResult{URL: resultURL, Page: processedPage, Success: true, Stage: "processing"}
}
func (p *Pipeline) sendErrorResult(u, stage, message string, retry bool) {
	result := &engmodels.CrawlResult{URL: u, Error: engmodels.NewCrawlError(u, stage, errors.New(message)), Success: false, Stage: stage, Retry: retry}
