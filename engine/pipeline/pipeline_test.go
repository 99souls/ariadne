...
func TestPipelineBasicFlow(t *testing.T) {
	...
	for r := range results {
		if r.Stage != "output" {
			t.Fatalf("expected final stage output got %s", r.Stage)
		}
		count++
	}
	if count != len(urls) {
...
func TestPipelineResourceCacheHit(t *testing.T) {
	...
	mgr, err := engresources.NewManager(cfg)
	if err != nil {
		t.Fatalf("rm: %v", err)
	}
	...
	if processed != len(urls) {
		t.Fatalf("expected %d got %d", len(urls), processed)
	}
	m := pl.Metrics()
	if m.StageMetrics["extraction"].Processed != 1 {
		t.Fatalf("expected 1 extraction")
	}
	if m.StageMetrics["cache"].Processed != 1 {
...
func TestPipelineResourceSpill(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(temp, "spill"))
	if err != nil {
		t.Fatalf("spill dir error: %v", err)
	}
	found := false
	for _, e := range entries {
		if strings.HasSuffix(e.Name(), ".spill.json") {
			found = true
			break
		}
	}
...
func TestPipelineCheckpointing(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(temp, "cp.log"))
	if err != nil {
		t.Fatalf("cp read: %v", err)
	}
	lines := strings.Split(strings.TrimSpace(string(data)), "\n")
	if len(lines) != len(urls) {
		t.Fatalf("expected %d cp entries got %d", len(urls), len(lines))
	}
}
